...
Le site vitrine sous `site/OmniEurope.Blazor.Showcase` est une application WebAssembly statique, publiable sans serveur applicatif et sous la même politique de sécurité stricte. Il présente le produit, la documentation, un explorateur de composants et un personnalisateur de thème. L'explorateur couvre les composants publiés en 23 familles et démontre chaque valeur de chaque paramètre énuméré ; le code affiché sous une démonstration est le fichier réellement compilé, relu depuis les ressources de l'assembly. Des gardes de la suite de tests verrouillent cette couverture, le rendu effectif de chaque démonstration, l'absence de fuite d'un littéral d'énumération dans le HTML rendu, la conformité CSP et le contraste des palettes.

Trois sondes de compatibilité complètent le catalogue : `samples/OmniEurope.Blazor.WasmSmoke` publie une application autonome dont le clic et la progression sont exercés dans Chromium, `samples/OmniEurope.Blazor.AutoSmoke` vérifie le prérendu puis l'hydratation et le clic dans Chromium, et `samples/OmniEurope.Blazor.HybridSmoke` est une application MAUI Windows exécutable qui charge les composants dans `BlazorWebView`. La CI exerce WebAssembly et Auto par CDP, et Hybrid par un auto-test exécuté dans WebView2 (clic, résultat, langue, titre, console) dont le résultat est publié sur la sortie standard de l'hôte, le runner n'exposant pas le port de débogage WebView2; les quatre exigent une console sans erreur; le catalogue possède en plus des contrôles navigateur de focus et de noms accessibles.
