...
- Vitrine publiée : `eng/Test-ShowcaseHost.ps1` sert la vitrine publiée (`dotnet publish site/OmniEurope.Blazor.Showcase -o artifacts/showcase-smoke`) avec sa CSP stricte et y lance deux sondes CDP. `Test-ShowcasePickerProbe.mjs` pilote les sélecteurs de date, d'heure et de date et heure par de vrais événements clavier et souris (grille, colonnes, Échap et retour du focus, appui dehors, un seul panneau ouvert, densité, clair et sombre, zéro violation CSP, console propre). `Test-ShowcaseDensityProbe.mjs` est le contrôle T22 du PLAN-008 : chaque page de la galerie en compacte puis en aérée, échec si un élément que la feuille du paquet dimensionne garde la même hauteur, exemptions motivées dans le script. Porte manuelle, hors CI ; en cas d'échec, la liste qu'imprime la sonde de densité est la liste à corriger, pas à exempter.
