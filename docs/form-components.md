...
- `OmniTextBox` rend le type demandé par `Type` (`Text`, `Email`, `Tel`, `Url` ou `Search`), ce qui choisit le clavier mobile et le remplissage automatique du navigateur. `Icon` (facultatif) pose une icône décorative au début du champ, en couleur atténuée et transparente au clic, et décale le texte après elle : la loupe d'un champ de recherche (`<Icon><OmniIcon Name="OmniIconName.Search" /></Icon>`). Le champ garde son propre nom accessible ; sans `Icon`, il est rendu seul, comme avant.
