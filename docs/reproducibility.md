...
La compilation active `Deterministic` dans `Directory.Build.props` et les dépendances sont verrouillées par les fichiers `packages.lock.json`. `global.json` demande le SDK `10.0.302` avec `rollForward: latestPatch` : il sélectionne donc le dernier patch installé de la bande de fonctionnalités `10.0.3xx`, sans verrouiller un binaire SDK exact.
