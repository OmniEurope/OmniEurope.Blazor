...
Dépôt : `OmniEurope/OmniEurope.Blazor`. Version déclarée dans `src/OmniEurope.Blazor/OmniEurope.Blazor.csproj` : `1.0.2`.
...
3. **Créer la Release.** Elle déclenche la publication.
   ...
   Tag : la version exacte du `.csproj`, sans préfixe (`1.0.2`). Cible : `main`. Le workflow compare le tag à la version du paquet et échoue en cas d'écart.
