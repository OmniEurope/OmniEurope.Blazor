...
- **Ce que la trace a répondu** : les cinq marqueurs sortent sur le runner, jusqu'à `webview-url-loading`, et six processus `msedgewebview2` sont vivants. WebView2 est donc bien créé et charge la page. Le défaut n'est ni le runtime, ni l'application, ni le démarrage.
- **Fausse piste écartée** : le message d'erreur du port, un timeout `HttpClient` de deux secondes, laissait croire que quelque chose écoutait sans répondre. Un port simplement fermé produit exactement le même message en local. Ce libellé ne porte aucune information, il ne faut rien en déduire.
...
- **Ce que le runner a répondu ensuite** : la trace montre `arguments="--remote-debugging-port=..."` côté fils, donc la variable arrive bien jusqu'au processus, et pourtant aucun processus navigateur ne la porte sur sa ligne de commande. Écrire la variable directement dans le bloc d'environnement du fils n'a donc pas suffi, et l'héritage est écarté comme explication.
- **Correctif retenu** : WebView2 lit aussi une valeur par exécutable sous la clé de stratégie Edge `HKCU\Software\Policies\Microsoft\Edge\WebView2\AdditionalBrowserArguments`, un chemin qui ne dépend d'aucune variable ni d'aucun héritage. La sonde la pose avant de lancer l'hôte et restaure exactement l'état antérieur dans son `finally`, cette valeur appartenant à la machine et non à l'exécution.
- **Vérifications faites avant livraison** : la clé seule, sans aucune variable d'environnement, ouvre le port (trace `arguments=""` et une cible `page` qui répond) ; les deux mécanismes ensemble ne dupliquent pas l'argument (un processus navigateur, une occurrence, une cible) ; et la valeur est bien absente après un passage vert.
- **Incertitude assumée** : la raison exacte pour laquelle la variable se perd sur `windows-latest` reste inconnue. Le relevé de ligne de commande pourrait aussi être un faux négatif si `CommandLine` revenait vide pour ces processus, auquel cas l'argument passerait mais le port serait refusé en aval, par exemple par une stratégie Edge. La voie registre couvre les deux premiers cas, pas ce dernier.
- **Verdict final** : la voie registre a échoué elle aussi sur le runner, avec la même signature. Trois mécanismes distincts, tous prouvés en local, n'ouvrent pas le port là-bas. L'explication restante cohérente avec tout est que l'image interdit le débogage distant de WebView2, ce qui n'est ni dans le code ni dans le dépôt.

### La sonde Hybrid exigeait une chose que le runner ne donne pas

- **Symptôme** : quatre correctifs successifs, chacun vert en local, chacun rouge sur le runner, sur un hôte qui pourtant démarre et rend sa page là-bas (les cinq marqueurs de démarrage sortent, six processus WebView2 vivants).
- **Cause de fond** : la sonde dépendait du port de débogage CDP de WebView2, une ressource que ne contrôlent ni le code ni le dépôt, et que l'image `windows-latest` refuse quel que soit le moyen de la demander. Corriger la manière de demander le port était la mauvaise cible depuis le début.
- **Correctif** : la sonde ne pilote plus l'hôte de l'extérieur. Quand `HYBRIDSMOKE_SELFTEST` est défini, l'hôte se teste de l'intérieur : `wwwroot/hybrid-smoke.js`, chargé avant Blazor pour capter les erreurs console dès le début, clique le bouton par un vrai événement DOM, attend le compteur, lit langue et titre, puis le .NET publie le résultat sur la sortie standard, que `eng/Test-HybridHost.ps1` lit et vérifie avec les mêmes assertions qu'avant. Aucun port, aucune variable WebView2, aucune clé registre.
- **Contrôles** : sortie brute de l'hôte affichant `selftest language="fr" title="Test hybride OmniEurope.Blazor" observed="1" errors=0` ; contrôle négatif avec un sélecteur cassé qui fait rougir la sonde avec `L'auto-test Hybrid a échoué dans WebView2 : Missing action element` et la sortie complète de l'hôte ; scan CSP vert sur le nouveau module ; suite unitaire 377/377.
- **Limite connue** : les erreurs console sont captées par `console.error`, `error` et `unhandledrejection` depuis le chargement de la page, pas par le protocole de débogage. Une erreur émise par le runtime WebView2 lui-même hors de ces canaux ne serait pas vue.
- **Leçon** : quand une preuve échoue sur le runner et passe partout ailleurs, se demander d'abord de quelle ressource de l'environnement elle dépend, avant de chercher pourquoi la demande de cette ressource échoue. Une sonde de CI ne doit dépendre que de ce que le dépôt contrôle.
...
- **Premier correctif, écarté après essai** : déplacer l'étape SBOM sur le job Windows, qui restaure le host Hybrid, en lui ajoutant un restore de la solution. Le job Windows a alors rendu `NU1403: Package content hash validation failed for Microsoft.NET.Sdk.WebAssembly.Pack.10.0.11`, sur trois projets WebAssembly, alors que le même restore passe sur Linux et en local. Déplacer l'inventaire vers un runner dont le cache NuGet est préchargé autrement a donc créé une panne à la place d'une autre.
- **Correctif retenu** : `eng/Restore-LockedPackages.ps1` télécharge les paquets épinglés par un fichier de verrou via des éléments `PackageDownload`, dans un projet jetable écrit hors du dépôt. Aucune évaluation du projet MAUI, aucun workload, aucune plateforme requise : le job Linux obtient les nuspec et les textes de licence dont l'inventaire a besoin, et l'étape SBOM reste où elle était.
- **Reproduction** : supprimer `~/.nuget/packages/microsoft.aspnetcore.components.webview/10.0.11` puis lancer `eng/Generate-Sbom.ps1` reproduit le message du runner mot pour mot. Lancer le nouveau script ramène les 60 paquets du verrou Hybrid, et la chaîne complète repasse au vert.
- **Leçon** : le besoin réel n'était pas de construire le projet manquant mais de disposer de ses paquets. `PackageDownload` répond exactement à ça et ne dépend ni du TFM ni du workload. Déplacer une étape vers un autre runner pour contourner un cache manquant échange un problème contre un autre, car chaque image préremplit son cache à sa façon.
- **Leçon complémentaire** : un projet sorti de la solution reste dans le périmètre des gardes qui balaient les fichiers du dépôt, pas dans celui des jobs qui compilent la solution. Tout ajout de ce genre doit se demander qui restaure ses paquets.

### Une étape verte qui échoue quand même

- **Symptôme** : l'étape CSP imprime ses trois lignes de succès, `CSP source scan passed: 1 files checked`, `CSP scanner fixtures passed`, `CSP source scan passed: 448 files checked`, puis `Error: Process completed with exit code 1`.
