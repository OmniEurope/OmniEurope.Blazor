...
- Rail d'icônes : les icônes gardent l'alignement du menu ouvert au lieu d'être centrées. Pile repliée : seul le libellé est masqué, l'icône reste, reconnue par `aria-hidden` ou `role="img"`. Pile qui défile : barre fine, non flottante, sous les boutons. En-tête de grille : séparation en dégradé, visible aussi quand l'en-tête collant défile. Badges au demi-rayon. `OmniLink` souligné au survol seulement.
