...
- Infobulles : elles paraissent après 300 ms de pointeur posé au lieu de tout de suite. Barre de chargement : le remplissage suit une seule courbe qui ralentit sans jamais s'arrêter (50 % en 3 s, 77 % en 10 s, 91 % en 30 s) et le balayage, fini, laisse son trait filer jusqu'au bout au lieu de se remplir. Notifications regroupées : la pile est une cascade où chaque carte se pose sous le titre de la précédente, trois titres visibles au plus derrière la carte de devant ; le compteur ferme la zone, erreurs comprises. Un cadre (`OmniCard`, `OmniSettingsTile`) peut devenir plus étroit que son contenu, et `OmniSelectBar` défile au lieu de se couper.
